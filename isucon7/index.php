...
$app->get('/php', function (Request $request, Response $response) {
    phpinfo();
});
...
$app->get('/message', function (Request $request, Response $response) {
    ...
    // );
    $stmt = $dbh->prepare(
        "SELECT message.*,name, display_name, avatar_icon ".
        "FROM message ".
    ...
    foreach ($rows as $row) {
        ...
        // $r['user'] = $stmt->fetch();
        $r['user'] = array("name" =>$row['name'],"display_name" =>$row['display_name'],"avatar_icon" =>$row['avatar_icon']);
...
//    set_time_limit(3);
    ...
    $stmt = $dbh->prepare(`SELECT channel.id,IFNULL(cnt.cnt,0)cnt
    FROM channel
    LEFT JOIN (
        ...
        WHERE IFNULL((
            ...
            WHERE haveread.user_id = ?
             AND haveread.channel_id =message.channel_id),0)<id
        GROUP BY channel_id
    ) cnt
     ON channel.id=cnt.channel_id
     ORDER BY channel.id`);
